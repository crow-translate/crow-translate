...
{
    return m_mainWindow->ocr()->language().isEmpty();
}

void OcrUninitializedTransition::onTransition(QEvent *)
{
    QMessageBox::critical(m_mainWindow, Ocr::tr("OCR language is not loaded"), Ocr::tr("You should set OCR language in the application settings"));
}
